...
if [ "${SERENITY_USE_GENEXT2FS:-0}" = "1" ]; then
    chmod -R a+rX mnt/
    chmod 666 mnt/etc/WindowServer.ini
    chmod 777 mnt/etc
    chmod 1777 mnt/tmp 2>/dev/null || true
    chmod -R a+rwX mnt/tmp mnt/var mnt/home 2>/dev/null || true
fi
...
ln -sf /bin/JarvisAssistant mnt/home/anon/Desktop/JARVIS\ Assistant
...
if [ "${SERENITY_USE_GENEXT2FS:-0}" = "1" ]; then
    chmod -R a+rwX mnt/home mnt/tmp mnt/var mnt/etc 2>/dev/null || true
    chmod -R a+rX mnt/bin mnt/res mnt/usr 2>/dev/null || true
fi
