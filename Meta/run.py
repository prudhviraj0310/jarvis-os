...
def set_up_gdb(config: Configuration):
    ...
    if config.enable_gdb:
        config.extra_arguments.extend(["-gdb", f"tcp:{config.host_ip}:11234"])
...
def set_up_network_hardware(config: Configuration):
    ...
    if config.machine_type.is_raspberry_pi():
        ...
        config.network_backend = f"user,id=breh,hostfwd=tcp:{config.host_ip}:18888-10.0.2.15:8888,\
hostfwd=tcp:{config.host_ip}:18823-10.0.2.15:23,\
hostfwd=tcp:{config.host_ip}:18000-10.0.2.15:8000,\
hostfwd=tcp:{config.host_ip}:12222-10.0.2.15:22"
